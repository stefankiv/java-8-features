package main.java.co.inventorsoft;
...
public class StreamHomework {
    ...
    public List<Person> extractTeenagers(final List<Person> people) {
        ...
        return people.stream().filter(person -> person != null && person.getAge() >= 13 && person.getAge() <= 19).collect(Collectors.toList());
    }
    ...
    public List<User> createUsers(final List<String> emails) {
        ...
        return emails.stream().filter(Objects::nonNull).distinct().map(User::new).collect(Collectors.toList());
    }
    ...
    public Map<String, User> groupByEmail(final List<User> users) {
        ...
        return users.stream().filter(Objects::nonNull).collect(Collectors.toMap(User::getEmail, Function.identity()));
    }
    ...
    public Map<Integer, List<Person>> groupByAge(final List<Person> people) {
        ...
        return people.stream().filter(Objects::nonNull).collect(Collectors.groupingBy(Person::getAge));
    }
    ...
    public String collectDistinctNames(final List<Person> people) {
        ...
        return "Distinct names: " + people.stream().filter(Objects::nonNull).map(Person::getName).distinct().collect(Collectors.joining(", ")) + "!";
    }
}
