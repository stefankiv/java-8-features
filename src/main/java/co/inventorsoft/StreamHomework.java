...
public class StreamHomework {
    ...
    public List<Person> extractTeenagers(final List<Person> people) {
        return people.stream().filter(person -> person.getAge() >= 13)
                .filter(person -> person.getAge() <=19) .collect(Collectors.toList());
    }
    ...
    public List<User> createUsers(final List<String> emails) {
        return emails.stream().filter(s -> s != null).distinct()
    ...
    public String collectDistinctNames(final List<Person> people) {
        ...
        return "Distinct names: "+ people.stream().map(Person::getName).distinct()
